...
def test_source_page_size_comes_from_immutable_pdf_not_viewport_extent(tmp_path) -> None:
    import fitz
    from pb_source_visibility_authority import SourceVisibilityProducer

    path = tmp_path / "page-size.pdf"
    doc = fitz.open()
    ...
    source = SourceVisibilityProducer(
        producer_method="component-page-size-test",
        producer_version="1",
    )
...
def test_source_page_size_rejects_wrong_hash(tmp_path) -> None:
    import fitz
    from pb_source_visibility_authority import SourceVisibilityProducer

    path = tmp_path / "page-size-hash.pdf"
    doc = fitz.open()
    ...
    source = SourceVisibilityProducer(
        producer_method="component-page-size-hash-test",
        producer_version="1",
    )
    published = source.ingest_native_pdf_bytes(
        document_id="page-size-hash",
        source_bytes=path.read_bytes(),
        source_locator=str(path),
        page_ids=("1",),
    )
