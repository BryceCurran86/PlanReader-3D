...
def test_physical_run_id_is_not_a_truth_input_for_the_number() -> None:
    """Two different opaque physical_run_id values over the identical source
    must resolve to the identical quantity -- the id is a selector key, not
    an authority input."""
    ...
    assert a.record.value == b.record.value == pytest.approx(4.25)
    assert a.record.dimension_chain_id == b.record.dimension_chain_id
