...
def test_future_public_resolver_is_selector_only_and_cannot_accept_raw_geometry() -> None:
    ...
    forbidden_selector_tokens = ("polygon","coord","voids","void_areas","opening_ids","deduction","complete","count","nearest","radius","confidence","deductible","net_area")
    ...
    forbidden_factory_names = {"gross_polygon","void_polygons","void_areas","opening_ids","deductible","claimed_complete","candidate_wall_ids","nearest_wall_id"}
...
def test_wall_local_boolean_subtraction_is_translation_rotation_and_reversal_invariant() -> None:
    ...
    voids = (box(2.0, 0.0, 4.0, 2.0), box(3.0, 1.0, 5.0, 3.0))
    ...
    reversed_voids = tuple(affinity.scale(v, xfact=-1.0, yfact=1.0, origin=(5.0, 0.0)) for v in voids)
