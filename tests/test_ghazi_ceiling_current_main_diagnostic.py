...
def test_ghazi_ceiling_current_main_diagnostic(tmp_path: Path) -> None:
    ...
    # Deep authority trace for the single drawing page.
    import fitz
    from pb_hosted_opening_instance_adapter import authoritative_floor_plan_viewports
    from pb_source_visibility_authority import SourceVisibilityProducer
    from pb_migration_provider_envelope import ProviderContext
    ...
    published = source_producer.ingest_native_pdf_bytes(
        document_id="ghazi-ceiling-diagnostic",
        source_bytes=payload_bytes,
        source_locator="memory://ghazi.pdf",
        page_ids=("167",),
    )
    pdf = fitz.open(stream=payload_bytes, filetype="pdf")
    try:
        page = pdf[166]
        segmented = authoritative_floor_plan_viewports(page, page_number=167)
        viewport_rows = []
        shadow_rows = []
        for vp in segmented:
            viewport_rows.append({
                "view_id": getattr(vp, "view_id", None),
                "status": getattr(vp, "status", None),
                "confidence": getattr(vp, "confidence", None),
                "bounding_box": getattr(vp, "bounding_box", None),
                "view_type": getattr(vp, "view_type", None),
            })
            if getattr(vp, "bounding_box", None) is None or not getattr(vp, "view_id", None):
                continue
            current = source_producer.published_snapshot_for_revision(
                published.revision.revision_id
            )
            viewport = ViewportEvidence(
                viewport_id=str(vp.view_id),
                document_id=current.revision.document_id,
                page_id="167",
                bbox=tuple(float(v) for v in vp.bounding_box),
                view_type="floor_plan",
                status=ViewportResolutionStatus.RESOLVED,
                evidence_ids=(),
            ...
            context = ProviderContext(
                run_id=f"diag:{current.snapshot.snapshot_id}:{vp.view_id}",
                workspace_id="diag",
                project_id="diag",
                document_id=current.revision.document_id,
                source_sha256=current.revision.source_sha256,
                ...
                current_revision_id=current.revision.revision_id,
                selected_pages=(166,),
                owned_viewport_ids=(str(vp.view_id),),
                evidence_snapshot_id=current.snapshot.snapshot_id,
                owned_page_numbers=(167,),
                viewport_page_ownership=((str(vp.view_id), 167),),
            )
            try:
                sr = run_source_owned_ceiling_lining_shadow(
                    source_visibility_producer=source_producer,
                    context=context,
                    viewport=viewport,
                    page_no=167,
                )
                shadow_rows.append({
                    ...
                    "finish_candidates": [
                        {
                            ...
                            "bbox": list(atom.geometry),
                            "status": atom.status.value,
                        }
                        for atom in sr.finish_candidates
                    ],
                    "scale_status": sr.scale_bridge.status.value,
                    "scale_reasons": list(sr.scale_bridge.reason_codes),
                    "scale_record_id": (
                        sr.scale_bridge.physical_scale_evidence.record_id
                        if sr.scale_bridge.physical_scale_evidence is not None
                        else None
                    ),
                    "room_area_quantities": [q.to_dict() if hasattr(q, "to_dict") else {
                        "quantity_id": q.quantity_id,
                        "value": q.value,
                        "status": q.status,
                        "authority": q.authority,
                        ...
                        "blocking_reasons": list(q.blocking_reasons),
                        "reason_codes": list(q.reason_codes),
                    } for q in sr.room_area_quantities],
                    "ceiling_quantities": [q.to_dict() if hasattr(q, "to_dict") else {
                        "quantity_id": q.quantity_id,
                        "value": q.value,
                        "status": q.status,
                        "authority": q.authority,
                    ...
                    } for q in sr.ceiling_quantities],
                })
            except Exception as exc:
                shadow_rows.append({
                    "view_id": vp.view_id,
    ...
    payload = {
        ...
        "floor_plan_viewports": viewport_rows,
        "shadow_traces": shadow_rows,
    }
