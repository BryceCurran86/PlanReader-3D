...
def _fixture(schedule_rows=VALID_MM_SCHEDULE):
    ...
    physical = PhysicalOpeningAuthority(src.authority()).prove_existence(observation_selector)
    assert physical.existence_record is not None

    binding_producer = ScheduleOpeningInstanceBindingProducer.from_source_visibility_producer(src)
    ...
    return src, binding_producer, binding_result, row_height_producer, height_selector
...
def test_explicit_metres_are_normalized_only_when_source_states_metres() -> None:
    src, binding, binding_result, row_height, selector = _fixture(VALID_EXPLICIT_M_SCHEDULE)
