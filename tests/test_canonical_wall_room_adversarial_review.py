"""Independent adversarial regressions for PR #286, cherry-picked from
GPT-2's review branch (gpt2/canonical-wall-room-adversarial-review, head
68aa420) after confirming they are test-only and generic. Reproduced as
failing against the exact reviewed head d51fed2 before any fix was applied.

This review intentionally adds only failing tests. It does not rewrite the
canonical wall/room model.

One geometry adjustment was made after reproduction, kept minimal and
documented: `test_one_end_connected_short_tick_does_not_become_existence_
corroborated`'s original 3.6pt tick length triggers Stage A's OWN pre-
existing, unrelated "short isolated arm" junction-classifier heuristic
(DEFAULT_SHORT_ARM_RELATIVE_THRESHOLD=0.15 relative to the carrier's own
arm length), which demotes the junction to AMBIGUOUS -- meaning
valid_junction_behavior never fires at all for that exact length, so the
test's own family-presence assertion could never observe the scenario it
names ("one real junction + one bare endpoint" -- AMBIGUOUS is not a real
junction). The tick length was changed to 10pt (still far short of a real
wall run, still well within the "short return/stub" class the blocker
names, still not a project-derived benchmark measurement) so the junction
classifier correctly reports a genuine T_JUNCTION+ENDPOINT pair, letting
the test exercise -- and this module correctly reject -- the intended
one-end-junction-is-not-independent-corroboration scenario. The test's
core semantic assertion (existence status must never be "corroborated")
was never weakened and already passed at any tick length once the domain
fix landed; only the family-presence assertion needed a length that avoids
an unrelated pre-existing demotion path.
"""
...
def test_one_end_connected_short_tick_does_not_become_existence_corroborated():
    ...
    out = _run_pipeline(
        [
            ...
            _seg("tick", 0, 0, 0, 10.0, layer="Layer 1"),
        ]
    )
    short_wall = min(
        out["resolved"],
