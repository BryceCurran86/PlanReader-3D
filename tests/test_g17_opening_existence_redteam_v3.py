...
# Exact #326 production head this independent red-team suite replays against.
_BRANCH_BASE_SHA = "3361458cd2b699fb940c0dc82d75c5c9c151ec7f"


def _rectangle_pdf_bytes(*, clip_prefix: bytes | None = None) -> bytes:
    """Build four real PDF vector segments forming only a closed rectangle.

    The rectangle intentionally has no wall-face continuation on either side.
    ``clip_prefix`` is inserted into the page content stream before the vector
    paths so clipping attacks exercise genuine PDF clipping, not test metadata.
    """
    ...
    if clip_prefix is not None:
        ...
        # q/Q scopes the clip to these four paths only.
...
def _ingest(payload: bytes, *, document_id: str):
    producer = SourceObservationProducer(
        producer_method="g17-redteam-v3",
...
def _visible_opening_pdf_bytes(
    *,
    angle_deg: float = 0.0,
    scale: float = 1.0,
    offset: tuple[float, float] = (0.0, 0.0),
    reverse_endpoints: bool = False,
    segment_order: str = "natural",
) -> bytes:
    """Six source-native segments: two-sided wall continuation + two jambs."""

    segments = [
        ((20.0, 100.0), (100.0, 100.0)),  # face A left continuation
        ((140.0, 100.0), (220.0, 100.0)),  # face A right continuation
        ((20.0, 110.0), (100.0, 110.0)),  # face B left continuation
        ((140.0, 110.0), (220.0, 110.0)),  # face B right continuation
        ((100.0, 100.0), (100.0, 110.0)),  # left jamb
        ((140.0, 100.0), (140.0, 110.0)),  # right jamb
    ]
    if segment_order == "reversed":
        segments = list(reversed(segments))
    elif segment_order == "rotated":
        segments = segments[2:] + segments[:2]

    doc = fitz.open()
    page = doc.new_page(width=700, height=700)
    for first, second in segments:
        a = _transform_point(first, angle_deg=angle_deg, scale=scale, offset=offset)
        b = _transform_point(second, angle_deg=angle_deg, scale=scale, offset=offset)
...
def _visible_selector(published, observation_id: str) -> ObservationSelector:
...
def _visible_opening_fixture(payload: bytes, *, document_id: str):
    producer = SourceVisibilityProducer(
        producer_method="g17-redteam-visible-positive",
        producer_version="1.0",
    )
    published = producer.ingest_native_pdf_bytes(
        document_id=document_id,
...
def test_unrelated_native_roots_cannot_certify_arbitrary_opening_geometry() -> None:
    """Attack 1: four unrelated roots must not certify caller-created geometry."""
...
def test_page2_native_roots_cannot_be_laundered_into_page1_existence() -> None:
    """Attack 2: a derived child cannot transfer page ownership from its roots."""

    producer, published, _, physical, records = _ingest(
        _two_page_rectangle_pdf_bytes(), document_id="g17-redteam-page-laundering"
    )
...
def test_viewport_none_roots_cannot_mint_vp1_authority() -> None:
    """Attack 3: roots with no viewport authority cannot mint vp-1 downstream."""

    producer, published, _, physical, records = _ingest(
        _rectangle_pdf_bytes(), document_id="g17-redteam-viewport-laundering"
    )
...
def test_fully_clipped_raw_vectors_cannot_establish_opening_existence() -> None:
    """Attack 4: raw vectors wholly outside a real rectangular clip are invisible."""
    ...
    producer, published, _, physical, records = _ingest(
        payload, document_id="g17-redteam-fully-clipped"
    )
...
def test_unresolved_clip_must_abstain_from_opening_existence() -> None:
    """Attack 5: a triangular W clip cannot be treated as a proven rectangle."""

    # The triangle's scissor overlaps the rectangle. A rectangular bounding box
    # therefore cannot establish which source segments are actually visible.
    ...
    producer, published, _, physical, records = _ingest(
        payload, document_id="g17-redteam-unresolved-clip"
    )
    snapshot_id = _publish_structure_from_native(producer, published, records)
    result = physical.prove_existence(selector(published, snapshot_id, "face-a"))
    _assert_no_positive_existence(result)
...
def test_closed_rectangle_without_wall_continuation_is_not_physical_opening() -> None:
    """Attack 6: a four-edge box is not a jamb-bounded wall interruption."""

    producer, published, _, physical, records = _ingest(
        _rectangle_pdf_bytes(), document_id="g17-redteam-box-only"
    )
    snapshot_id = _publish_structure_from_native(producer, published, records)
    result = physical.prove_existence(selector(published, snapshot_id, "face-a"))
    _assert_no_positive_existence(result)
...
def test_two_face_gap_without_two_jambs_never_resolves_existence() -> None:
    """Attack 7: two interruption claims without jamb support stay non-positive."""
...
def test_derived_only_semantic_evidence_never_establishes_existence(kind: str) -> None:
    """Attack 8: semantic-looking derived evidence is never structural truth."""
...
def test_shared_or_overlapping_lineage_cannot_manufacture_independence() -> None:
    """Attack 9: overlapping native support sets are one corroboration source."""
...
def test_semantic_existence_cannot_cross_downstream_firewall() -> None:
    """Attack 11: legitimate visible-native existence must not cross the firewall.

    Positive control uses SourceVisibilityProducer over six source-native line
    segments with wall continuation on both sides of one common gap. Do NOT reuse
    the unsafe make_resolved() fixture (Attack 1 negative control).
    """

    _, published, _, physical = _visible_opening_fixture(
        _visible_opening_pdf_bytes(),
        document_id="g17-redteam-attack11-positive",
    )
    assert len(published.visible_observation_ids) == 6

    results = [
        physical.prove_existence(_visible_selector(published, observation_id))
    ...
    left = _visible_selector(published, published.visible_observation_ids[0])
    right = _visible_selector(published, published.visible_observation_ids[1])
    identity = physical.compare_identity(left, right)
...
@pytest.mark.parametrize(
    ("angle_deg", "scale", "offset", "reverse_endpoints", "segment_order"),
    (
        (0.0, 1.0, (0.0, 0.0), False, "natural"),
        (0.0, 1.0, (55.0, -20.0), False, "natural"),
        (90.0, 1.0, (350.0, 20.0), False, "natural"),
        (0.0, 2.0, (40.0, 30.0), False, "natural"),
        (0.0, 1.0, (0.0, 0.0), True, "natural"),
        (0.0, 1.0, (0.0, 0.0), False, "reversed"),
        (0.0, 1.0, (0.0, 0.0), False, "rotated"),
    ),
)
def test_legitimate_visible_opening_is_metamorphically_stable(
    angle_deg: float,
    scale: float,
    offset: tuple[float, float],
    ...
    """Metamorphic positive control: same structural existence under transforms."""
    ...
    _, published, _, physical = _visible_opening_fixture(
        ...
        document_id=(
            f"g17-redteam-meta-{angle_deg}-{scale}-{offset[0]}-"
            f"{int(reverse_endpoints)}-{segment_order}"
        ),
    )
    results = [
        physical.prove_existence(_visible_selector(published, observation_id))
...
def test_redteam_branch_is_bound_to_reviewed_governing_main() -> None:
    """Document the immutable #326 review baseline inside the attack suite."""

    assert _BRANCH_BASE_SHA == "3361458cd2b699fb940c0dc82d75c5c9c151ec7f"
