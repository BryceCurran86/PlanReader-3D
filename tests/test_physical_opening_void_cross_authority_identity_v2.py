...
def _build_two_opening_chain():
    ...
    for opening_selector, opening in zip(opening_selectors, opening_results):
        ...
        host_selectors.append(host_selector)
        ...
        schedule_records.append(schedule.record)
...
def test_two_real_openings_positive_individually_but_cross_wire_fails_closed() -> None:
    ...
    # Positive controls prove both records are individually complete and valid.
    positives = [
    ...
    # Cross-wire exact same-lineage opening A/B.  Numeric dimensions are deliberately
    # identical, so only exact producer-owned physical identity can reject the join.
