...
def _owned_metadata(**extra) -> dict:
    """Current-revision/snapshot/source-SHA ownership every evidence atom
    (existence or coverage) must carry to be usable, matching
    pb_physical_wall_existence_authority's own atom-ownership contract."""
    return {
        "revision_id": "R1",
        "evidence_snapshot_id": "evsnap",
        "source_sha256": SHA,
...
def _existence_atoms_for_wall(wall_id: str) -> tuple[EvidenceAtom, EvidenceAtom]:
    """Wall-scoped existence atoms: real evidence_ids are content-hashed and
    therefore already wall-specific, so pooling multiple walls' atoms in one
    evidence_atoms sequence requires genuinely distinct ids per wall (a bare
    shared "u2-ev"/"pair-ev" literal across two walls would be a fixture-only
    same-id collision under collision-safe existence recomputation)."""
    return (
        _atom(f"u2-ev-{wall_id}", KIND_PHYSICAL_WALL, metadata=_owned_metadata(wall_candidate_id=wall_id)),
        _atom(f"pair-ev-{wall_id}", FAMILY_PAIRED_WALL_FACES, metadata=_owned_metadata(wall_candidate_id=wall_id)),
    )


def _corroborate(walls: list[WallCandidate], *, supporting=None, conflicting=()) -> list[WallCandidate]:
    return [
        replace(
            wall,
            supporting_evidence_ids=supporting if supporting is not None else (f"u2-ev-{wall.candidate_id}", f"pair-ev-{wall.candidate_id}"),
...
def _existence_atoms():
    """Back-compat single-wall helper: real callers should prefer
    ``_existence_atoms_for_wall`` (see ``_roles``, which builds one pair per
    wall automatically). Kept for tests that construct one wall directly."""
    return _existence_atoms_for_wall("w1")


def _atoms_for_walls(walls) -> tuple[EvidenceAtom, ...]:
    """One owned existence-atom pair per wall, keyed by its real candidate_id."""
    return tuple(atom for wall in walls for atom in _existence_atoms_for_wall(wall.candidate_id))
...
def _roles(
    ...
    *,
    ...
    per_wall_atoms = tuple(
        atom for wall in walls for atom in _existence_atoms_for_wall(wall.candidate_id)
    )
    atoms = per_wall_atoms + extra_atoms + coverage
    ids = tuple(
        dict.fromkeys(
            [
...
def test_candidate_existence_cannot_receive_firm_role() -> None:
    ...
    # Only one evidence family (native linework) per wall -- no independent
    # second domain (paired faces), so existence must stay CANDIDATE.
    walls = [replace(wall, supporting_evidence_ids=(f"u2-ev-{wall.candidate_id}",)) for wall in walls]
    atoms = tuple(_existence_atoms_for_wall(wall.candidate_id)[0] for wall in walls)
    ...
    assert wall_physical_existence_status(
        walls[0], evidence_atoms=atoms, document=document, viewport=viewport, context=context
...
def test_reversed_centerline_keeps_semantic_role() -> None:
    ...
    forward = resolve_wall_boundary_roles(
        walls=walls, graph=graph, edge_id_to_wall_id=edge_map, evidence_atoms=atoms, document=document, viewport=viewport, context=context
    )
    reversed_walls = [
    ...
    backward = resolve_wall_boundary_roles(
        walls=reversed_walls, graph=graph, edge_id_to_wall_id=edge_map, evidence_atoms=atoms, document=document, viewport=viewport, context=context
    )
...
def test_shuffled_walls_are_deterministic() -> None:
    ...
    b = resolve_wall_boundary_roles(
        walls=shuffled, graph=graph, edge_id_to_wall_id=edge_map, evidence_atoms=atoms, document=document, viewport=viewport, context=context
    )
