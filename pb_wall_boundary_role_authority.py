...
def _coverage_atom_ownership_reasons(
    atom: _AtomLike,
    *,
    ...
    """Same ownership discipline as ``pb_physical_wall_existence_authority``'s
    atom-ownership checks: exact document/page/viewport identity plus current
    revision, evidence snapshot, and source SHA. A coverage atom missing or
    disagreeing on any of these cannot confer architectural exterior/interior
    authority -- matching a stale/wrong document/SHA/revision/snapshot/
    viewport atom the same way the existence authority already fails closed
    on stale supporting evidence.
    """
    reasons: list[str] = []
    if str(_atom_field(atom, "document_id") or "") != document.document_id:
        reasons.append("viewport_coverage_document_mismatch")
    if str(_atom_field(atom, "page_id") or "") != viewport.page_id:
        reasons.append("viewport_coverage_page_mismatch")

    metadata = _atom_field(atom, "metadata") or {}
    atom_vp = str(metadata.get("viewport_id") or _atom_field(atom, "viewport_id") or "")
    if atom_vp != viewport.viewport_id:
        reasons.append("viewport_coverage_viewport_mismatch")

    if not context.revision_id or not context.current_revision_id:
        reasons.append("viewport_coverage_context_revision_unbound")
    elif context.revision_id != context.current_revision_id:
        reasons.append("viewport_coverage_context_revision_stale")

    atom_revision = str(metadata.get("revision_id") or "").strip()
    if not atom_revision:
        reasons.append("viewport_coverage_revision_unproven")
    elif atom_revision != context.revision_id:
        reasons.append("viewport_coverage_revision_stale")

    atom_snapshot = str(metadata.get("evidence_snapshot_id") or "").strip()
    if not atom_snapshot:
        reasons.append("viewport_coverage_snapshot_unproven")
    elif atom_snapshot != context.evidence_snapshot_id:
        reasons.append("viewport_coverage_snapshot_mismatch")

    atom_sha = str(metadata.get("source_sha256") or "").strip().lower()
    if not atom_sha:
        reasons.append("viewport_coverage_source_sha_unproven")
    elif not (atom_sha == document.source_sha256 == context.source_sha256):
        reasons.append("viewport_coverage_source_sha_mismatch")
...
def resolve_viewport_coverage_authority(
    *,
    ...
    """Independent, provenance-bound coverage proof.

    Crop/topology alone never establishes this, and neither does a coverage
    atom whose document/page/viewport/revision/snapshot/source-SHA ownership
    does not match the current context: such an atom is rejected before its
    ``coverage_status`` is ever consulted, exactly as a stale/foreign
    evidence atom is rejected for physical-wall existence.

    Returns ``(coverage_complete, reason_codes)``. Conflicting coverage atoms
    (after ownership filtering) fail closed (incomplete). Missing or
    entirely-rejected atoms are incomplete.
    """
    statuses: list[str] = []
    reasons: list[str] = []
    for atom in coverage_atoms:
        if str(_atom_field(atom, "kind") or "") != VIEWPORT_COVERAGE_KIND:
            continue
        ownership_reasons = _coverage_atom_ownership_reasons(
            atom, document=document, viewport=viewport, context=context
        )
...
def resolve_wall_boundary_roles(
    *,
    ...
    ``context`` is required (no default): both physical-wall existence and
    viewport-coverage authority are current-revision/snapshot/source-SHA
    bound, matching ``pb_physical_wall_existence_authority``'s own contract.
    """
