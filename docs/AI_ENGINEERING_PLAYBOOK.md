...
- `pb_wall_boundary_role_authority.resolve_wall_boundary_roles` (2026-09-14 integration onto post-#288 main, from Cursor's `5df6261` wall-role hardening): shadow only, role independent of thickness/length/scale/commercial publishing/W6 room-count. `context: ProviderContext` is required -- `wall_physical_existence_status` now needs it (the #288 remediation added it after Cursor's original branch forked), and `resolve_viewport_coverage_authority` binds coverage atoms to it too. A topologically-unbounded face stays `UNKNOWN` unless a `viewport_coverage` atom proves `coverage_status="complete"` AND matches the current document/page/viewport/revision/evidence-snapshot/source-SHA -- a coverage atom from another document/SHA/revision/snapshot/viewport fails exactly like a missing one (see `tests/test_wall_boundary_role_coverage_provenance.py`). Explicit exterior labels (courtyard/lightwell) and covered-open-space labels (verandah/porch/carport/loggia) remain independent of the coverage gate. On all four real-drawing fixtures with no coverage-evidence pipeline wired yet, every wall abstains to `UNKNOWN`/`CONFLICT` -- zero `EXTERNAL`/`INTERNAL_PARTITION` is the correct, expected result, not a defect.
