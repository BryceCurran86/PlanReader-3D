...
**Lane:** Opening dimensions (width / height). Physical opening identity from merged PR #331 is an upstream prerequisite and remains out of scope here.

## 1. Decision

`existence ≠ identity ≠ width ≠ height ≠ type ≠ host`.

Width and height must be independently proveable. G17 existence alone must not invent dimensions. Schedule dimensions alone must not invent physical openings or assign instance geometry without separately proven type/instance binding.

Merged #331 establishes physical opening identity only. It does not authorize width, height, host binding, physical void, deductions, commercial publication, or JobHub publication.
...
`pb_physical_opening_authority.PhysicalOpeningAuthority.capabilities()` on post-#331 main:
...
No in-repo reference to GitHub issue #330 / CMap / ToUnicode was found in the original dimension audit. Attack 14 remains a required fail-closed case for corrupt native text → must not silently FIRM.
...
2. Merged #331 physical-instance identity is now available as a **prerequisite consumer input** for assigning plan/elevation/schedule dimensions to an instance; this lane must not re-derive or weaken identity.
...
- Inputs: producer-owned G17 existence + merged #331 identity + figured/elevation/schedule evidence atoms under document/revision/page/viewport ownership.
...
- Does not unlock host binding, universe completeness, physical void, deductions, commercial, or JobHub.

**Stop for review.** No production implementation in this PR.

## 5. Benchmark observations that must not drive design

Development score **31/61 = 50.82%**. Gold, mappings, tolerances, project names, “typical door” sizes, and detector confidence bands must not drive the algorithm.

## 6. Explicit non-goals (this PR)

- Reimplementing or changing merged opening identity (#331)
