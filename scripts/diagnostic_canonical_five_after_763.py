    ...
    for attempt in range(4):
        try:
            request = urllib.request.Request(
                url,
                headers={"User-Agent": "PlanReader-canonical-current-main/1.0"},
            )
...
result = {
    "diagnostic": "canonical_five_current_main",
    "production_sha": PRODUCTION_SHA,
    "projects": rows,
    "scored_project_count": len(scored),
    "totals": totals,
}
print("CANONICAL_FIVE_AFTER_763=" + json.dumps(result, sort_keys=True), flush=True)
