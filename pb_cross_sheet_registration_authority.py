...
corresponds to evidence on another sheet (e.g. elevation, section):

- Requires exact document/revision/source/snapshot lineage.
- Existence of a matching ``physical_element_id`` in independent
  ``PhysicalWallCandidateAuthority`` scopes on both the source and target
  page is *necessary* but never *sufficient*. Same mark string (e.g.
  "W-01"), the same caller-selected id, or mere per-page coexistence never
  mint registration by themselves.
- A positive registration additionally requires a real, source-backed
  correspondence proof: an explicit drafting cross-reference callout
  (``pb_cross_sheet_callout_evidence``) printed near the element on the
  source page whose referenced sheet code matches the target page's own
  unambiguous sheet code. No caller-constructible proof object exists in
  this module.
- Ambiguous element matches (more than one candidate sharing the selected
  id on either page) fail closed rather than taking the first match.
...
CROSS_SHEET_REGISTRATION_SCHEMA_VERSION = "2.0.0"
...
class CrossSheetRegistrationProducer:
    ...
    def publish(
        ...
        tgt_matches, tgt_scope_res = self._matching_records(selector, selector.target_page_id, selector.physical_element_id)
        if tgt_matches is None:
            return self._store(
                selector,
                _abstained(
        ...
        tgt_wc = getattr(tgt_matches[0], "wall_candidate", None)
        src_view = str(getattr(src_wc, "view_type", None) or "plan").lower()
        tgt_view = str(getattr(tgt_wc, "view_type", None) or "elevation").lower()

        # Element existence on both pages is necessary but never
        # sufficient -- a real source-backed correspondence callout is
        # required before this can become CORROBORATED.
        ...
        geometry_bbox = (min(xs), min(ys), max(xs), max(ys))
