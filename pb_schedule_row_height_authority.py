...
def _unit_tokens(text: str) -> frozenset[str]:
    ...
    if _MM_RE.search(text or ""):
        units.add("mm")
    # Run the metre check after mm; the negative lookbehind prevents the
    # trailing m in "mm" from being interpreted as metres.
...
class ScheduleRowHeightEvidence:
    """Source-proven rough-opening height semantics for one schedule row."""
...
class ScheduleRowHeightProducer:
    ...
    def publish_scope(self, selector: ScheduleRowHeightSelector) -> ScheduleRowHeightResult:
        ...
        published = self._visibility_producer.published_snapshot_for_revision(
        ...
        if not page_rows:
            return self._store(
                key,
                _blocked(EvidenceResolutionStatus.ABSTAINED, HEIGHT_ROW_UNAVAILABLE),
            )
        try:
        ...
        if len(matches) != 1:
            return self._store(
                key,
                _blocked(EvidenceResolutionStatus.CONFLICT, HEIGHT_ROW_AMBIGUOUS),
            )
        ...
        if entry.height_mm is None:
            return self._store(
                key,
                _blocked(EvidenceResolutionStatus.ABSTAINED, HEIGHT_FIELD_UNAVAILABLE),
            )
        # Only an explicitly labelled rough/structural-opening dimension is
        # the physical wall void height. Frame/leaf/clear/generic dimensions
        # remain useful diagnostics but cannot become this authority.
        if entry.dimension_basis != "rough_opening" or not entry.basis_source:
            return self._store(
                key,
                _blocked(EvidenceResolutionStatus.ABSTAINED, HEIGHT_BASIS_UNPROVEN),
            )
        ...
        header_cells = [
            cell.strip() for cell in str(header_row.get("text", "")).split("\t")
        ]
        target_cells = [
            cell.strip() for cell in str(target_row.get("text", "")).split("\t")
        ]
        ...
        if not unit_tokens:
            return self._store(
                key,
                _blocked(EvidenceResolutionStatus.ABSTAINED, HEIGHT_UNITS_UNPROVEN),
            )
        if len(unit_tokens) != 1:
            return self._store(
                key,
                _blocked(EvidenceResolutionStatus.CONFLICT, HEIGHT_UNITS_CONFLICT),
            )
