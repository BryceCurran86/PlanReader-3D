...
ITEM 19A (this module): FAIL-CLOSED pending source-derived binding authority.
  - Caller-provided WallFinishAssignment objects NEVER produce CORROBORATED results.
  - All publishes return ABSTAINED with diagnostic reason.
  - No face multipliers applied to caller-selected targets.
  - Caller assignments remain valid for diagnostics/UI/provisional paths only.

ITEM 19B (future): Producer-owned WallFinishFaceBindingAuthority will prove:
  - Exact finish/material by extracting from authenticated source evidence.
  - Exact wall-face binding (left_face vs right_face vs both_faces with proof).
  - Source-derived trade scope association.
  - Authority will propagate net wall area only when finish binding is proven.

CURRENT ARCHITECTURE (Item 19A fail-closed):
1. Exact Physical Wall Identity:
   - Finishes are addressed by exact physical_wall_id.
...
2. Net Area Respect (does NOT imply finish authority):
   - NetWallBooleanUnionAuthority proves: authenticated net wall area ONLY.
   - It does NOT prove: finish existence, material, trade, face count, both-faces.
   - Multiplying by face_multiplier requires Item 19B source-derived evidence.
3. Fail-Closed on Caller Assignments:
   - WallFinishAssignment accepted for structural API compatibility.
   - All caller-selected face_targets (left/right/both) are treated diagnostically.
   - Result: EvidenceResolutionStatus.ABSTAINED, record=None.
   - Reason: "item_19a_waiting_for_source_derived_finish_binding_authority".
4. Scope Summary Completeness:
   - Aggregations across physical walls always fail closed (is_scope_complete=False).
   - total_trade_area_m2 = None until Item 19B source-derived binding exists.
