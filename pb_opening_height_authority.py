...
class OpeningHeightProducer:
    ...
    def publish_scope(self, selector: OpeningHeightSelector) -> OpeningHeightResult:
        ...
        if (
            ...
            return self._store(
                key,
                _blocked(binding_result.status, *binding_result.reason_codes),
            )
        ...
        if (
            row_height_result.status is not EvidenceResolutionStatus.CORROBORATED
            or row_height_result.evidence is None
        ):
            return self._store(
                key,
                _blocked(row_height_result.status, *row_height_result.reason_codes),
            )
        row_height = row_height_result.evidence
        if (
            row_height.schedule_page_id != binding.schedule_page_id
            or tuple(sorted(row_height.schedule_row_observation_ids))
